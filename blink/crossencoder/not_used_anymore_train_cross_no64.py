...
def modify(context_input, candidate_input, params, mode = "train"):
...
def evaluate(reranker, eval_dataloader, device, logger, context_length, zeshel=False, silent=True, train=True, input = None):
    ...
    if nb_eval_examples > 0:
        if train:
            normalized_eval_accuracy = eval_accuracy / nb_eval_examples# 여기 고치면 됨
...
def main(params):
    ...
    # print(label_input)
    print("train data size", context_input.shape)
    ...
    context_input = modify(context_input, candidate_input, params, mode = "train")
    if params["zeshel"]:
    ...
    context_input = modify(context_input, candidate_input, params, mode = "valid")
    ...
    context_input = modify(context_input, candidate_input, params, mode = "test")
    if params["zeshel"]:
