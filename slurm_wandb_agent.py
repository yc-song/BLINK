...
def main(args):
    ...
    file_str['srun.sh'] = "\n".join((
        "",
        f"output=`wandb agent --count 1 {complete_sweep_id}`",
        "run_id=${output: (-11):8}",
        "val_acc=${output: (-2):2}",
        f'echo "$output"',
        f'echo "$run_id"',
        f'echo "$val_acc"',
        f"if  ((10#$val_acc>20))" , #adjust val_acc to rerun
        f"then",
    ...
    file_str['sbatch_run.sh'] = "\n".join((
        ...
        f"decoding=`grep -o \'\"decoding\": [^,]*\' ./models/zeshel/crossencoder/$1/training_params.json | grep -o \'[^ ]*$\'`",
        "echo \"lr=${lr}, dim_red=${dim_red}, layers=${layers}, train_batch_size=${train_batch_size}\"",
        "python /home/jongsong/BLINK/blink/crossencoder/train_cross.py --dim_red=${dim_red} --layers=${layers} --learning_rate=${lr} --train_batch_size=${train_batch_size} --resume=True --run_id=$1",
        f"  sbatch {file_path['sbatch.sh']}",
