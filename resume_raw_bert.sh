...
echo "lr=${lr}"
python_output=$(python /home/jongsong/BLINK/blink/crossencoder/train_cross.py --learning_rate=${lr} --resume=True --run_id=$1 --num_train_epochs=${num_train_epochs} --train_batch_size 8 --eval_batch_size 8 --wandb "raw_context_text" --save True --train_size -1 --valid_size -1 --architecture raw_context_text --add_linear True)
echo $python_output
